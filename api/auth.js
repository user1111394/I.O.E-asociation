...
export default async function handler(req, res) {
  ...
  const DB_SECRET = process.env.FIREBASE_DB_SECRET; // sama seperti yang dipakai api/member.js
  if (!DB_URL) return res.status(500).json({ error: 'FIREBASE_DB_URL tidak ditemukan' });

  // Helper: bikin URL Firebase yang otomatis nempel auth secret (kalau ada),
  // supaya konsisten dengan pola yang sudah terbukti berhasil di api/member.js
  function dbPath(path) {
    return `${DB_URL}${path}.json${DB_SECRET ? `?auth=${DB_SECRET}` : ''}`;
  }
  ...
  if (action === 'register') {
    ...
    try {
      ...
      const checkRes = await fetch(dbPath(`/usernames/${cleanUsername}`));
      const existingId = await checkRes.json();

      if (existingId && typeof existingId === 'string') {
        const existingAccRes = await fetch(dbPath(`/accounts/${existingId}`));
        ...
        await fetch(dbPath(`/usernames/${cleanUsername}`), { method: 'DELETE' });
      } else if (existingId) {
      ...
      let attempts = 0;
      while (attempts < 5) {
        const idCheckRes = await fetch(dbPath(`/accounts/${memberId}`));
      ...
      const saveAccRes = await fetch(dbPath(`/accounts/${memberId}`), {
        method: 'PUT',
      ...
      if (!saveAccRes.ok) {
        let detail = '';
        try { detail = JSON.stringify(await saveAccRes.json()); } catch (e) { detail = await saveAccRes.text().catch(() => ''); }
        return res.status(500).json({
          error: 'Gagal menyimpan data akun, coba lagi',
          debugStatus: saveAccRes.status,
          debugDetail: detail,
        });
      }

      const saveUserRes = await fetch(dbPath(`/usernames/${cleanUsername}`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
      ...
      if (!saveUserRes.ok) {
        ...
        await fetch(dbPath(`/accounts/${memberId}`), { method: 'DELETE' });
        let detail = '';
        try { detail = JSON.stringify(await saveUserRes.json()); } catch (e) { detail = await saveUserRes.text().catch(() => ''); }
        return res.status(500).json({
          error: 'Gagal menyimpan username, coba lagi',
          debugStatus: saveUserRes.status,
          debugDetail: detail,
        });
      }
  ...
  if (action === 'login') {
    ...
    try {
      const idRes = await fetch(dbPath(`/usernames/${cleanUsername}`));
      const memberId = await idRes.json();
      if (!memberId) {
        return res.status(401).json({ error: 'Username atau password salah' });
      }

      const accRes = await fetch(dbPath(`/accounts/${memberId}`));
      ...
      const sessionToken = generateSessionToken();
      await fetch(dbPath(`/accounts/${memberId}/currentSession`), {
        method: 'PUT',
        headers: { 'Content-Type': 'application/json' },
