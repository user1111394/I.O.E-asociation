...
export default async function handler(req, res) {
  ...
  if (action === 'ban-member') {
    ...
    // Proteksi dasar: wajib ada admin token yang cocok dengan ADMIN_MASTER_KEY
    // (pola yang sama seperti admin-token.js yang sudah ada)
    const ADMIN_KEY = process.env.ADMIN_MASTER_KEY;
    if (!ADMIN_KEY || adminToken !== ADMIN_KEY) {
  ...
  if (action === 'verify-superadmin') {
    ...
    // Validasi format master-key (cegah SQL injection) — hanya alphanumeric, dash, underscore
    if (!/^[a-zA-Z0-9\-_]+$/.test(masterKey)) {
      return res.status(400).json({ error: 'Format master key tidak valid' });
    }
