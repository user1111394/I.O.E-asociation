...
//
// Pakai endpoint /set/{key}/{value} (bentuk path, bukan body) supaya bisa
// menambahkan query param ?NX yang didukung native oleh Upstash REST API.
async function kvSetNX(key, value) {
  const encodedValue = encodeURIComponent(JSON.stringify(value));
  const r = await fetch(`${KV_URL}/set/${encodeURIComponent(key)}/${encodedValue}?NX=true`, {
    method: 'POST',
    headers: { Authorization: `Bearer ${KV_TOKEN}` },
  });
