...
export default async function handler(req, res) {
  ...
  // ══════════════════════════════════════
  // CLAIM-QUEST — Member klaim reward quest event sendiri (otomatis, tanpa
  // redeem code). Terkunci: 1 quest cuma bisa diklaim SEKALI per akun.
  // Storage: questclaimed:{eventId}:{questId}:{memberId} -> true
  // ══════════════════════════════════════
  if (action === 'claim-quest') {
    const { memberId, sessionToken, eventId, questId, rewardCoin } = req.body;
    ...
    try {
      ...
      if (alreadyClaimed) {
        return res.status(409).json({ error: 'Quest ini sudah pernah diklaim' });
      }

      // Tandai sudah diklaim DULU sebelum menambah koin — mencegah race
      // condition kalau member klik tombol berkali-kali dengan sangat cepat.
      const marked = await kvSet(claimKey, true);
      ...
      return res.status(200).json({ success: true, coins: newAmount, rewardClaimed: rewardNum });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  }
  ...
  // ══════════════════════════════════════
  // CHECK-QUEST-CLAIMED — Member cek quest mana saja yang sudah diklaim
  // (dipanggil saat event.html pertama kali render, supaya tombol yang
  // sudah diklaim tampil "Selesai" bukan "Klaim" lagi)
  // ══════════════════════════════════════
