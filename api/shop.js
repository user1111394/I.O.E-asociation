...
export default async function handler(req, res) {
  ...
  if (action === 'claim-quest') {
    ...
    try {
      ...
      if (type === 'submit-link') {
        if (!submittedLink) return res.status(400).json({ error: 'Link video wajib diisi' });
        ...
        if (!proofImageUrl) return res.status(400).json({ error: 'Screenshot bukti wajib diupload' });
        status = 'pending';
      }
