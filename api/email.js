...
function generateTagSuffix() {
  // Generate random suffix: 4 karakter random + timestamp pendek
...
export default async function handler(req, res) {
  ...
  if (action === 'register-tag') {
    ...
    try {
      ...
      // Generate tag unik
      // Format: ⸢I.O.E⸥{username}#{suffix}
      const tagSuffix = generateTagSuffix();
      const specialTag = `⸢I.O.E⸥${cleanUsername}#${tagSuffix}`;
