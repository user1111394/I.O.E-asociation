...
const PETS = {
  ...
  goldfish:      { name: 'Ikan Mas',        tier: 'common',    biome: 'coast',     color: 0xff8c00, sellPrice: 6 },
...
const BARN_CAPACITY_BASE = 6; // slot kandang dasar, bisa di-upgrade nanti (belum ada upgrade di versi awal ini)

function defaultPetBarnState() {
  return { pets: [] }; // { instanceId, petId, happiness, lastFedAt, lastCleanedAt, lastYieldCollectedAt }
}
...
export default async function handler(req, res) {
  ...
  if (action === 'attempt-catch') {
    ...
    try {
      ...
      if (qteSuccess) {
        // Berhasil: spot respawn normal, hewan masuk ke Pet Barn.
        ...
        if (barnState.pets.length < BARN_CAPACITY_BASE) {
          barnState.pets.push({
          ...
          // Kandang penuh: tetap dianggap tertangkap secara peristiwa,
          ...
          return res.status(200).json({
            ...
            message: `Kandang penuh! ${petData.name} yang tertangkap tidak bisa masuk kandang.`,
          });
        }
      } else {
  ...
  if (action === 'get-petbarn-state') {
    try {
      ...
      const petsWithComputed = state.pets.map(pet => ({
        ...pet,
      ...
      return res.status(200).json({
        state: { pets: petsWithComputed }, animalCoins, pets: PETS,
        foodCost: PET_FOOD_COST, barnCapacity: BARN_CAPACITY_BASE,
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
