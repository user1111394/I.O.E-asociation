...
export default async function handler(req, res) {
  ...
  if (action === 'attempt-catch') {
    ...
    try {
      ...
      if (qteSuccess) {
        ...
        let barnState = await kvGet(`petbarn:${memberId}`);
        if (!barnState) barnState = defaultPetBarnState();
  ...
  if (action === 'get-petbarn-state') {
    try {
      let state = await kvGet(petBarnKey);
      if (!state) state = defaultPetBarnState();

      const animalCoins = await kvGet(`animalcoins:${memberId}`) || 0;
      const levelData = BARN_LEVELS[state.level || 1];
