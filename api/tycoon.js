...
// Helper terpusat: kembalikan array plots dengan `progress`/`stage`
// terhitung untuk SETIAP plot, dipakai SEMUA action Garden yang
// mengembalikan `state` ke klien (bukan cuma get-garden-state).
//
// BUG YANG DIPERBAIKI: sebelumnya cuma get-garden-state yang menyisipkan
// progress/stage; action lain (plant-seed, water-plant, fertilize-plant,
// treat-pest, harvest-plant, upgrade-garden) mengembalikan plots MENTAH
// tanpa field itu. Di klien, tanaman dengan stage undefined otomatis
// digambar sebagai scale minimum (seolah baru ditanam) — jadi begitu
// SATU plot dipanen lewat harvest-plant, SEMUA plot lain yang datang
// dalam response yang sama ikut kelihatan "reset" ke tahap awal secara
// VISUAL saja (progress asli di Redis tidak pernah berubah, cuma
// representasi yang dikirim ke klien tidak lengkap).
function plotsWithComputedProgress(plots, season) {
...
// ══════════════════════════════════════
// CATCH A PET — dunia 3D TERPISAH dari Wild Harvest (dikonfirmasi user:
// kalau digabung bakal berat karena hewan butuh AI kabur, beda logic
// sama sekali dari dig spot yang diam). File frontend: catchapet.html.
// Backend numpang di file ini juga (demi hemat slot API Vercel).
// Mata uang baru: ANIMAL COIN (animalcoins:{memberId}).
//
// 5 bioma PERSIS SAMA seperti Wild Harvest (nama sama persis di
// dokumen asli), tapi didefinisikan ULANG dengan namespace PET_ terpisah
// supaya kalau salah satu diubah nanti tidak saling memengaruhi.
// ══════════════════════════════════════

const PET_BIOMES = {
  meadow:    { name: 'Sunny Meadows',    angleStart: 0,   angleEnd: 72,  color: 0x8fd888 },
  forest:    { name: 'Whispering Woods', angleStart: 72,  angleEnd: 144, color: 0x3a7a4a },
  coast:     { name: 'Crystal Coast',    angleStart: 144, angleEnd: 216, color: 0xf0e0b0 },
  snow:      { name: 'Frostbite Valley', angleStart: 216, angleEnd: 288, color: 0xeef4f8 },
  highlands: { name: 'Ember Highlands',  angleStart: 288, angleEnd: 360, color: 0x9a6248 },
};
const PET_WORLD_RADIUS_MIN = 15;
const PET_WORLD_RADIUS_MAX = 260;

// 5 tier alat tangkap (lasso/perangkap), pola SAMA PERSIS shovel Wild
// Harvest. `catchRadius` = jarak maksimal QTE tangkap bisa dimulai.
const CATCH_TOOL_TIERS = {
  1: { name: 'Cotton Rope',     maxDurability: 10,       canCatchTier: ['common'], catchRadius: 3.0 },
  2: { name: 'Silk Glow Lasso', maxDurability: 30,       canCatchTier: ['common', 'uncommon'], catchRadius: 3.6 },
  3: { name: 'Ironvine Lasso',  maxDurability: 60,       canCatchTier: ['common', 'uncommon', 'rare'], catchRadius: 4.2 },
  4: { name: 'Starlight Lasso', maxDurability: 120,      canCatchTier: ['common', 'uncommon', 'rare', 'legendary'], catchRadius: 4.8 },
  5: { name: 'Dragonscale Net', maxDurability: Infinity, canCatchTier: ['common', 'uncommon', 'rare', 'legendary', 'mythic'], catchRadius: 5.5 },
};
const CATCH_TOOL_UPGRADE_COST = { 2: 100, 3: 500, 4: 2000, 5: 8000 };
const CATCH_TOOL_REPAIR_COST_PER_DURABILITY = 5;

// 20 spesies (14 dari dokumen 3D-model-pet.txt + 6 variasi tambahan,
// sesuai keputusan user). `livestockYield` cuma diisi untuk hewan
// TERNAK (sapi/kambing/ayam/lebah) — hasil yang dipanen di kandang
// (Pet Barn) dan dijual dapat Animal Coin. Hewan pet/fantasy TIDAK
// punya field ini (cuma koleksi + happiness, sesuai keputusan user).
const PETS = {
  chicken:       { name: 'Ayam',            tier: 'common',    biome: 'meadow',    color: 0xa0522d, sellPrice: 8,   livestockYield: { itemName: 'Telur', minutesPerYield: 30, sellPrice: 4 } },
  cow:           { name: 'Sapi',            tier: 'uncommon',  biome: 'meadow',    color: 0xf5f5f5, sellPrice: 40,  livestockYield: { itemName: 'Susu Sapi', minutesPerYield: 60, sellPrice: 12 } },
  goat:          { name: 'Kambing',         tier: 'common',    biome: 'highlands', color: 0xd2b48c, sellPrice: 10,  livestockYield: { itemName: 'Susu Kambing', minutesPerYield: 50, sellPrice: 9 } },
  goldfish:      { name: 'Ikan Mas',        tier: 'common',    biome: 'river',     color: 0xff8c00, sellPrice: 6 },
  cat:           { name: 'Kucing',          tier: 'uncommon',  biome: 'forest',    color: 0xe8d0a8, sellPrice: 30 },
  dog:           { name: 'Anjing',          tier: 'uncommon',  biome: 'meadow',    color: 0xd4a373, sellPrice: 35 },
  rabbit:        { name: 'Kelinci',         tier: 'common',    biome: 'meadow',    color: 0xf5f5f5, sellPrice: 12 },
  hamster:       { name: 'Hamster',         tier: 'common',    biome: 'forest',    color: 0xd2a679, sellPrice: 7 },
  dragon:        { name: 'Naga Kecil',      tier: 'legendary', biome: 'highlands', color: 0xdc143c, sellPrice: 500 },
  unicorn:       { name: 'Unicorn',         tier: 'legendary', biome: 'meadow',    color: 0xf5f5f5, sellPrice: 500 },
  firefox:       { name: 'Rubah Api',       tier: 'rare',      biome: 'highlands', color: 0xff4500, sellPrice: 100 },
  beetle:        { name: 'Kumbang Raksasa', tier: 'rare',      biome: 'forest',    color: 0x2f2f2f, sellPrice: 100 },
  icecat:        { name: 'Kucing Es',       tier: 'rare',      biome: 'snow',      color: 0x1e90ff, sellPrice: 100 },
  bee:           { name: 'Lebah',           tier: 'common',    biome: 'meadow',    color: 0xffd700, sellPrice: 6,   livestockYield: { itemName: 'Madu', minutesPerYield: 45, sellPrice: 10 } },
  chicken_white: { name: 'Ayam Putih',      tier: 'common',    biome: 'meadow',    color: 0xf5f5f5, sellPrice: 8,   livestockYield: { itemName: 'Telur', minutesPerYield: 30, sellPrice: 4 } },
  cow_brown:     { name: 'Sapi Coklat',     tier: 'uncommon',  biome: 'highlands', color: 0xd2b48c, sellPrice: 40,  livestockYield: { itemName: 'Susu Sapi', minutesPerYield: 60, sellPrice: 12 } },
  cat_black:     { name: 'Kucing Hitam',    tier: 'uncommon',  biome: 'snow',      color: 0x1a1a1a, sellPrice: 30 },
  rabbit_brown:  { name: 'Kelinci Coklat',  tier: 'common',    biome: 'forest',    color: 0x8b7355, sellPrice: 12 },
  dragon_ice:    { name: 'Naga Es',         tier: 'legendary', biome: 'snow',      color: 0x1e90ff, sellPrice: 500 },
  penguin:       { name: 'Pinguin',         tier: 'uncommon',  biome: 'snow',      color: 0x1a1a1a, sellPrice: 35 },
  sheep:         { name: 'Domba',           tier: 'uncommon',  biome: 'meadow',    color: 0xf0ead6, sellPrice: 25,  livestockYield: { itemName: 'Wol', minutesPerYield: 40, sellPrice: 8 } },
  octopus:       { name: 'Gurita',          tier: 'rare',      biome: 'coast',     color: 0xb84a8c, sellPrice: 120 },
  wasp:          { name: 'Tawon',           tier: 'rare',      biome: 'forest',    color: 0xf0c020, sellPrice: 110 },
};

const PET_TIER_RESPAWN_MINUTES = { common: 5, uncommon: 10, rare: 20, legendary: 60, mythic: 1440 };
// QTE tangkap: jarum bergerak 0-100 di bar, harus tekan pas di zona
// hijau tengah. Makin langka = zona makin sempit & jarum makin cepat.
const QTE_GREEN_ZONE_WIDTH = { common: 40, uncommon: 30, rare: 22, legendary: 14, mythic: 8 };
const QTE_NEEDLE_SPEED = { common: 1.4, uncommon: 1.7, rare: 2.0, legendary: 2.4, mythic: 2.8 };
// Kalau player LARI (bukan jalan), semua hewan dalam radius ini kabur
// (1 aturan simpel untuk semua hewan, sesuai keputusan user).
const PET_FLEE_RADIUS_ON_RUN = 12;

// Daftar spawn TETAP (bukan random posisi tiap load), pola sama persis
// dig spot Wild Harvest — supaya semua member lihat titik yang sama.
// Ikan (biome 'river') dipisah jadi 6 spot SUNGAI tersendiri (ID
// "river_0".."river_5"), TIDAK ikut campur dengan 30 spot bioma darat —
// ini yang bikin ikan cuma bisa ditangkap dari sungai, sesuai keputusan
// user, karena posisi 3D-nya (dihitung klien) mengikuti jalur sungai.
function buildPetSpawnLayout() {
  const layout = {};
  for (const [biomeId] of Object.entries(PET_BIOMES)) {
    const petsInBiome = Object.entries(PETS).filter(([, p]) => p.biome === biomeId).map(([id]) => id);
    for (let i = 0; i < 6; i++) {
      layout[`${biomeId}_${i}`] = { biome: biomeId, petPool: petsInBiome };
    }
  }
  const riverPets = Object.entries(PETS).filter(([, p]) => p.biome === 'river').map(([id]) => id);
  for (let i = 0; i < 6; i++) {
    layout[`river_${i}`] = { biome: 'river', petPool: riverPets };
  }
  return layout;
}
const PET_SPAWN_LAYOUT = buildPetSpawnLayout();

function defaultCatchPetState() {
  const spawns = {};
  for (const spotId of Object.keys(PET_SPAWN_LAYOUT)) {
    spawns[spotId] = { petId: null, fledAt: null };
  }
  return {
    toolTier: 1,
    toolDurability: CATCH_TOOL_TIERS[1].maxDurability,
    spawns,
  };
}

function refreshPetSpawns(state) {
  const now = Date.now();
  for (const [spotId, layout] of Object.entries(PET_SPAWN_LAYOUT)) {
    const spot = state.spawns[spotId];
    if (!spot) { state.spawns[spotId] = { petId: null, fledAt: null }; continue; }
    const needsNewPet = !spot.petId || (spot.fledAt !== null && now >= spot.fledAt);
    if (needsNewPet) {
      spot.petId = pickRandom(layout.petPool);
      spot.fledAt = null;
    }
  }
  return state;
...
// ══════════════════════════════════════
// PET BARN — kandang aktif (petbarn.html, halaman terpisah lagi dari
// catchapet.html, tapi backend numpang di file ini juga). Hewan yang
// ditangkap masuk ke sini: dipiara (makan/bersih/elus utk happiness),
// dan KHUSUS hewan ternak (livestockYield) bisa dipanen hasilnya utk
// dijual dapat Animal Coin.
// ══════════════════════════════════════

const PET_FOOD_COST = 15; // Animal Coin per kali kasih makan
const PET_HAPPINESS_PER_FEED = 15;
const PET_HAPPINESS_PER_CLEAN = 10;
const PET_HAPPINESS_PER_PET = 5; // elus
const PET_HAPPINESS_DECAY_PER_HOUR = 2; // happiness berkurang perlahan seiring waktu kalau tidak diurus
const MAX_HAPPINESS = 100;

// Level kandang — pola SAMA PERSIS GARDEN_LEVELS (harga naik per level,
// kapasitas bertambah). `pets` = slot untuk hewan darat biasa (ternak +
// pet + fantasy). Kolam ikan (`fishPondCapacity`) TERPISAH dari slot ini
// (sesuai keputusan: ikan punya "kandang" sendiri berupa kolam, bukan
// berbagi slot dengan hewan darat).
const BARN_LEVELS = {
  1: { pets: 6,  fishPondCapacity: 3,  upgradeCost: 0 },
  2: { pets: 10, fishPondCapacity: 5,  upgradeCost: 800 },
  3: { pets: 16, fishPondCapacity: 8,  upgradeCost: 2500 },
  4: { pets: 24, fishPondCapacity: 12, upgradeCost: 6000 },
  5: { pets: 36, fishPondCapacity: 18, upgradeCost: 12000 },
};

function defaultPetBarnState() {
  return {
    level: 1,
    pets: [],     // hewan darat: { instanceId, petId, happiness, lastFedAt, lastCleanedAt, lastYieldCollectedAt }
    fishPond: [], // ikan: struktur sama, tapi disimpan terpisah & dibatasi fishPondCapacity sendiri
  };
}

// Ambil state Pet Barn dari Redis DAN migrasi otomatis kalau formatnya
// dari versi lama (sebelum ada upgrade kandang & kolam ikan terpisah,
// waktu itu state cuma { pets: [...] } tanpa `level`/`fishPond`).
// SEMUA action Pet Barn wajib pakai fungsi ini (bukan kvGet langsung),
// supaya tidak ada lagi celah `.map()`/`.find()` di field yang belum
// ada pada state lama.
async function loadBarnState(memberId) {
  let state = await kvGet(`petbarn:${memberId}`);
  if (!state) return defaultPetBarnState();
  if (!Array.isArray(state.fishPond)) state.fishPond = [];
  if (!state.level) state.level = 1;
  if (!Array.isArray(state.pets)) state.pets = [];

  // Migrasi tambahan: ikan (goldfish) yang KETERLANJUR tersimpan di
  // `pets` (ditangkap sebelum kolam ikan terpisah ada) dipindah ke
  // `fishPond` di sini, supaya tampil di slot kolam yang benar alih-alih
  // nyasar di slot kandang darat.
  const fishInPets = state.pets.filter(p => p.petId === 'goldfish');
  if (fishInPets.length > 0) {
    state.pets = state.pets.filter(p => p.petId !== 'goldfish');
    state.fishPond.push(...fishInPets);
  }

  return state;
}

function computeCurrentHappiness(pet) {
  const lastUpdate = Math.max(pet.lastFedAt || 0, pet.lastCleanedAt || 0, pet.happinessSetAt || 0);
  const hoursPassed = (Date.now() - lastUpdate) / (1000 * 60 * 60);
  const decayed = pet.happiness - hoursPassed * PET_HAPPINESS_DECAY_PER_HOUR;
  return Math.max(0, Math.min(MAX_HAPPINESS, Math.round(decayed)));
}

// Hitung berapa banyak hasil ternak yang siap dipanen sejak terakhir
// dikumpulkan, berdasarkan minutesPerYield spesies itu (idle-progress
// style, sama seperti applyRefineryProgress).
function computePendingYield(pet) {
  const petData = PETS[pet.petId];
  if (!petData || !petData.livestockYield) return 0;
  const minutesPassed = (Date.now() - (pet.lastYieldCollectedAt || pet.caughtAt || Date.now())) / (1000 * 60);
  return Math.floor(minutesPassed / petData.livestockYield.minutesPerYield);
}
...
export default async function handler(req, res) {
  ...
  // ══════════════════════════════════════
  // GRANT-SEED-COIN — superadmin kasih Seed Coin langsung ke member
  // (giveaway), pola SAMA PERSIS seperti grant-ore-coin di atas, cuma
  // beda mata uang target (`seedcoins:{memberId}`, milik Wild
  // Harvest + Garden, bukan Ore Coin milik Tycoon).
  // ══════════════════════════════════════
  if (action === 'grant-seed-coin') {
    const { adminToken, memberId: targetMemberId, amount } = req.body;
    ...
    const amountNum = Number(amount);
    if (!Number.isFinite(amountNum) || amountNum < 1) {
      return res.status(400).json({ error: 'Jumlah Seed Coin harus angka lebih dari 0' });
    }
    ...
    try {
      ...
      const currentSeedCoins = await kvGet(`seedcoins:${targetMemberId}`) || 0;
      const newSeedCoins = currentSeedCoins + amountNum;
      await kvSet(`seedcoins:${targetMemberId}`, newSeedCoins);

      return res.status(200).json({ success: true, seedCoins: newSeedCoins });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  }
  ...
  if (action === 'get-garden-state') {
    try {
      ...
      return res.status(200).json({
        state: { ...state, plots: plotsWithComputedProgress(state.plots, season) },
  ...
  if (action === 'plant-seed') {
    ...
    try {
      ...
      return res.status(200).json({
        success: true,
        state: { ...gardenState, plots: plotsWithComputedProgress(gardenState.plots, season) },
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  ...
  if (action === 'fertilize-plant') {
    ...
    try {
      ...
      const season = await getOrInitSeason();
      return res.status(200).json({
        success: true, seedCoins: seedCoins - fert.cost,
        state: { ...state, plots: plotsWithComputedProgress(state.plots, season) },
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
  ...
  if (action === 'treat-pest') {
    ...
    try {
      ...
      const season = await getOrInitSeason();
      return res.status(200).json({
        success: true, seedCoins: seedCoins - PEST_TREATMENT_COST,
        state: { ...state, plots: plotsWithComputedProgress(state.plots, season) },
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
  ...
  if (action === 'harvest-plant') {
    ...
    try {
      ...
      return res.status(200).json({
        ...
        seedCoins: seedCoins + earnedSeedCoins,
        state: { ...state, plots: plotsWithComputedProgress(state.plots, season) },
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
  ...
  if (action === 'upgrade-garden') {
    try {
      ...
      const season = await getOrInitSeason();
      return res.status(200).json({
        success: true, seedCoins: seedCoins - levelData.upgradeCost,
        state: { ...state, plots: plotsWithComputedProgress(state.plots, season) },
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
  ...
  const catchPetKey = `catchpet:${memberId}`;

  if (action === 'get-catchpet-state') {
    try {
      let state = await kvGet(catchPetKey);
      if (!state) state = defaultCatchPetState();
      state = refreshPetSpawns(state);
      await kvSet(catchPetKey, state);

      const animalCoins = await kvGet(`animalcoins:${memberId}`) || 0;

      return res.status(200).json({
        state, animalCoins,
        biomes: PET_BIOMES, pets: PETS, toolTiers: CATCH_TOOL_TIERS,
        toolUpgradeCost: CATCH_TOOL_UPGRADE_COST,
        worldRadiusMin: PET_WORLD_RADIUS_MIN, worldRadiusMax: PET_WORLD_RADIUS_MAX,
        greenZoneWidth: QTE_GREEN_ZONE_WIDTH, needleSpeed: QTE_NEEDLE_SPEED,
        fleeRadiusOnRun: PET_FLEE_RADIUS_ON_RUN,
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  ...
  // FLEE-NEARBY-PETS — dipanggil klien tiap kali player mendeteksi diri
  // sendiri LARI (bukan jalan). Server yang tentukan spot mana saja yang
  // dalam radius (klien kirim posisi player), supaya tidak bisa dicurangi
  // dari klien. Spot yang kabur disembunyikan sampai respawn seperti biasa.
  if (action === 'flee-nearby-pets') {
    const { playerX, playerZ, spotPositions } = req.body;
    // spotPositions: { spotId: {x, z} }, dikirim klien karena posisi
    // spot itu deterministik dari ID (dihitung di klien, sama pola
    // seperti digSpotWorldPosition Wild Harvest) — server tidak perlu
    // simpan koordinat 3D sendiri, cukup dikirim ulang oleh klien untuk
    // dicocokkan jaraknya dengan validasi radius di sini.
    if (typeof playerX !== 'number' || typeof playerZ !== 'number' || !spotPositions) {
      return res.status(400).json({ error: 'Data posisi tidak lengkap' });
    }
    ...
    try {
      ...
      let fledCount = 0;
      for (const [spotId, pos] of Object.entries(spotPositions)) {
        const spot = state.spawns[spotId];
        if (!spot || !spot.petId || spot.fledAt !== null) continue;
        const dist = Math.sqrt((playerX - pos.x) ** 2 + (playerZ - pos.z) ** 2);
        if (dist <= PET_FLEE_RADIUS_ON_RUN) {
          const petData = PETS[spot.petId];
          spot.fledAt = Date.now() + PET_TIER_RESPAWN_MINUTES[petData.tier] * 60 * 1000;
          fledCount++;
        }
      }

      await kvSet(catchPetKey, state);
      return res.status(200).json({ success: true, state, fledCount });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  }

  // ATTEMPT-CATCH — hasil QTE (menang/kalah) ditentukan KLIEN (karena
  // ini soal timing jarum visual real-time yang tidak praktis divalidasi
  // presisi di server), TAPI server tetap validasi semua hal yang bisa
  // dicurangi: jarak ke spot (pakai posisi dikirim klien, sama pola
  // seperti flee-nearby-pets), tier alat vs tier hewan, durability, dan
  // status spot (bukan sedang kabur). Durability berkurang TIAP KALI
  // percobaan (menang ataupun kalah), sama persis pola shovel gali.
  if (action === 'attempt-catch') {
    const { spotId, playerX, playerZ, qteSuccess } = req.body;
    if (!spotId || !PET_SPAWN_LAYOUT[spotId]) {
      return res.status(400).json({ error: 'Spot tidak dikenali' });
    }
    if (typeof playerX !== 'number' || typeof playerZ !== 'number') {
      return res.status(400).json({ error: 'Posisi pemain tidak valid' });
    }
    ...
    try {
      ...
      const spot = state.spawns[spotId];
      if (spot.fledAt !== null && Date.now() < spot.fledAt) {
        return res.status(400).json({ error: 'Hewan ini sedang kabur, tunggu sampai muncul lagi' });
      }

      const petData = PETS[spot.petId];
      const tool = CATCH_TOOL_TIERS[state.toolTier];
      if (!tool.canCatchTier.includes(petData.tier)) {
        return res.status(400).json({ error: `Alat kamu belum cukup kuat untuk menangkap ${petData.name} (tier ${petData.tier})` });
      }
      if (state.toolDurability <= 0) {
        return res.status(400).json({ error: 'Alat tangkap kamu sudah rusak, perbaiki dulu' });
      }

      // Durability berkurang tiap percobaan, berhasil ataupun gagal.
      if (state.toolDurability !== Infinity) {
        state.toolDurability = Math.max(0, state.toolDurability - 1);
      }

      const respawnMinutes = PET_TIER_RESPAWN_MINUTES[petData.tier];
      let caughtPetId = null;

      if (qteSuccess) {
        // Berhasil: spot respawn normal, hewan masuk ke Pet Barn — ikan
        // masuk ke `fishPond` (kapasitas sendiri), hewan darat lainnya
        // masuk ke `pets` (kapasitas sesuai level kandang).
        spot.fledAt = Date.now() + respawnMinutes * 60 * 1000;
        caughtPetId = spot.petId;
        const isFish = caughtPetId === 'goldfish';

        let barnState = await loadBarnState(memberId);
        const levelData = BARN_LEVELS[barnState.level || 1];
        const targetArray = isFish ? barnState.fishPond : barnState.pets;
        const capacity = isFish ? levelData.fishPondCapacity : levelData.pets;

        if (targetArray.length < capacity) {
          targetArray.push({
            instanceId: `pet_${Date.now()}_${Math.floor(Math.random() * 10000)}`,
            petId: caughtPetId,
            happiness: 70, // mulai dengan happiness cukup, bukan 0 (biar tidak langsung "sedih" pas baru ditangkap)
            lastFedAt: Date.now(),
            lastCleanedAt: Date.now(),
            happinessSetAt: Date.now(),
            lastYieldCollectedAt: Date.now(),
            caughtAt: Date.now(),
          });
          await kvSet(`petbarn:${memberId}`, barnState);
        } else {
          // Kandang/kolam penuh: tetap dianggap tertangkap secara peristiwa,
          // tapi TIDAK ditambahkan ke barn (biar tidak hilang diam-diam,
          // klien perlu diberitahu supaya member sadar harus buka slot).
          caughtPetId = null;
          return res.status(200).json({
            success: true, caught: false, barnFull: true, state,
            message: isFish
              ? `Kolam ikan penuh! ${petData.name} yang tertangkap tidak bisa masuk kolam.`
              : `Kandang penuh! ${petData.name} yang tertangkap tidak bisa masuk kandang.`,
          });
        }
      } else {
        // Gagal (meleset QTE): hewan kabur, sama seperti efek lari.
        spot.fledAt = Date.now() + respawnMinutes * 60 * 1000;
      }
      ...
      return res.status(200).json({
        success: true, caught: !!caughtPetId, petId: caughtPetId,
        toolDurability: state.toolDurability, state,
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  ...
  if (action === 'upgrade-catch-tool') {
    try {
      let state = await kvGet(catchPetKey);
      if (!state) state = defaultCatchPetState();

      const nextTier = state.toolTier + 1;
      const tierData = CATCH_TOOL_TIERS[nextTier];
      if (!tierData) return res.status(400).json({ error: 'Alat sudah di tier maksimal' });

      const cost = CATCH_TOOL_UPGRADE_COST[nextTier];
      const animalCoins = await kvGet(`animalcoins:${memberId}`) || 0;
      if (animalCoins < cost) return res.status(400).json({ error: 'Animal Coin tidak cukup' });

      await kvSet(`animalcoins:${memberId}`, animalCoins - cost);
      state.toolTier = nextTier;
      state.toolDurability = tierData.maxDurability;
      await kvSet(catchPetKey, state);

      return res.status(200).json({ success: true, state, animalCoins: animalCoins - cost });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  }

  if (action === 'repair-catch-tool') {
    try {
      let state = await kvGet(catchPetKey);
      if (!state) state = defaultCatchPetState();

      const tierData = CATCH_TOOL_TIERS[state.toolTier];
      if (tierData.maxDurability === Infinity) {
        return res.status(400).json({ error: 'Alat ini tidak pernah rusak, tidak perlu diperbaiki' });
      }
      const missing = tierData.maxDurability - state.toolDurability;
      if (missing <= 0) return res.status(400).json({ error: 'Durability alat sudah penuh' });

      const cost = missing * CATCH_TOOL_REPAIR_COST_PER_DURABILITY;
      const animalCoins = await kvGet(`animalcoins:${memberId}`) || 0;
      if (animalCoins < cost) return res.status(400).json({ error: 'Animal Coin tidak cukup' });
  ...
  // ══════════════════════════════════════════════════════════════════
  // PET BARN — actions (kandang aktif, halaman terpisah petbarn.html,
  // backend numpang di file ini juga)
  // ══════════════════════════════════════════════════════════════════

  const petBarnKey = `petbarn:${memberId}`;

  // Cari instance hewan di kandang, entah itu di `pets` (hewan darat)
  // atau `fishPond` (ikan) — dipakai semua action perawatan supaya tidak
  // perlu tahu dulu di array mana hewan itu berada.
  function findPetInstance(state, instanceId) {
    return state.pets.find(p => p.instanceId === instanceId)
      || state.fishPond.find(p => p.instanceId === instanceId);
  }

  if (action === 'get-petbarn-state') {
    try {
      const state = await loadBarnState(memberId);
      // Simpan hasil migrasi (kalau ada perubahan seperti ikan yang
      // dipindah dari `pets` ke `fishPond`) supaya tidak perlu migrasi
      // ulang tiap request — sekali pindah, permanen di Redis.
      await kvSet(`petbarn:${memberId}`, state);

      const animalCoins = await kvGet(`animalcoins:${memberId}`) || 0;
      const levelData = BARN_LEVELS[state.level];

      const withComputed = (pet) => ({
        ...pet,
        currentHappiness: computeCurrentHappiness(pet),
        pendingYield: computePendingYield(pet),
      });

      return res.status(200).json({
        state: {
          level: state.level,
          pets: state.pets.map(withComputed),
          fishPond: state.fishPond.map(withComputed),
        },
        animalCoins, pets: PETS,
        foodCost: PET_FOOD_COST,
        barnCapacity: levelData.pets,
        fishPondCapacity: levelData.fishPondCapacity,
        barnLevels: BARN_LEVELS,
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  ...
  // UPGRADE-BARN — naik 1 level, tambah kapasitas pets & fishPond
  // sekaligus (pola sama persis upgrade-garden).
  if (action === 'upgrade-barn') {
    try {
      let state = await loadBarnState(memberId);

      const nextLevel = (state.level || 1) + 1;
      const levelData = BARN_LEVELS[nextLevel];
      if (!levelData) return res.status(400).json({ error: 'Kandang sudah di level maksimal' });

      const animalCoins = await kvGet(`animalcoins:${memberId}`) || 0;
      if (animalCoins < levelData.upgradeCost) return res.status(400).json({ error: 'Animal Coin tidak cukup' });

      await kvSet(`animalcoins:${memberId}`, animalCoins - levelData.upgradeCost);
      state.level = nextLevel;
      await kvSet(petBarnKey, state);

      return res.status(200).json({
        success: true, animalCoins: animalCoins - levelData.upgradeCost,
        state: { level: state.level, pets: state.pets, fishPond: state.fishPond },
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  ...
  if (action === 'feed-pet') {
    ...
    try {
      let state = await loadBarnState(memberId);
      const pet = findPetInstance(state, instanceId);
      if (!pet) return res.status(400).json({ error: 'Hewan tidak ditemukan di kandang' });

      const animalCoins = await kvGet(`animalcoins:${memberId}`) || 0;
      if (animalCoins < PET_FOOD_COST) return res.status(400).json({ error: 'Animal Coin tidak cukup' });

      await kvSet(`animalcoins:${memberId}`, animalCoins - PET_FOOD_COST);
      const current = computeCurrentHappiness(pet);
      pet.happiness = Math.min(MAX_HAPPINESS, current + PET_HAPPINESS_PER_FEED);
      pet.lastFedAt = Date.now();
      pet.happinessSetAt = Date.now();
      await kvSet(petBarnKey, state);

      return res.status(200).json({ success: true, state, animalCoins: animalCoins - PET_FOOD_COST });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
  }
  ...
  if (action === 'clean-pet') {
    ...
    try {
      ...
      const current = computeCurrentHappiness(pet);
      pet.happiness = Math.min(MAX_HAPPINESS, current + PET_HAPPINESS_PER_CLEAN);
      pet.lastCleanedAt = Date.now();
      pet.happinessSetAt = Date.now();
      await kvSet(petBarnKey, state);
  ...
  if (action === 'pet-the-pet') { // elus
    ...
    try {
      ...
      const current = computeCurrentHappiness(pet);
      pet.happiness = Math.min(MAX_HAPPINESS, current + PET_HAPPINESS_PER_PET);
      pet.happinessSetAt = Date.now();
      await kvSet(petBarnKey, state);
  ...
  // COLLECT-YIELD — panen hasil ternak (susu/telur/madu) yang sudah
  // terkumpul, LANGSUNG dijual jadi Animal Coin (bukan disimpan sebagai
  // item, karena tidak ada sistem inventory barang di Catch a Pet —
  // lebih simpel, sesuai pola "harvest-plant langsung jual" di Garden).
  if (action === 'collect-yield') {
    const { instanceId } = req.body;
    try {
      let state = await loadBarnState(memberId);
      const pet = findPetInstance(state, instanceId);
      ...
      const petData = PETS[pet.petId];
      if (!petData.livestockYield) return res.status(400).json({ error: 'Hewan ini bukan ternak, tidak ada hasil untuk dipanen' });

      const pendingYield = computePendingYield(pet);
      if (pendingYield <= 0) return res.status(400).json({ error: 'Belum ada hasil yang siap dipanen' });

      const earnedAnimalCoins = pendingYield * petData.livestockYield.sellPrice;
      const animalCoins = await kvGet(`animalcoins:${memberId}`) || 0;
      await kvSet(`animalcoins:${memberId}`, animalCoins + earnedAnimalCoins);

      pet.lastYieldCollectedAt = Date.now();
      await kvSet(petBarnKey, state);

      return res.status(200).json({
        success: true, yieldCount: pendingYield, yieldName: petData.livestockYield.itemName,
        earnedAnimalCoins, animalCoins: animalCoins + earnedAnimalCoins, state,
      });
    } catch (e) {
      return res.status(500).json({ error: e.message });
    }
