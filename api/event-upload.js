...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    // ── Mode: galeri musik broadcast (Admin Abuse) ──
    if (uploadType === 'broadcast-music') {
      if (!/^audio\//i.test(contentType || '')) {
        return res.status(400).json({ error: 'File musik harus berupa audio' });
      }

      const dataUrl = `data:${contentType};base64,${fileDataBase64}`;

      const uploadResult = await cloudinary.uploader.upload(dataUrl, {
        folder: 'ioe-broadcast',
        public_id: fileName.replace(/\.[^/.]+$/, ''),
        resource_type: 'video', // Cloudinary menyimpan audio di bawah resource_type 'video'
        overwrite: true,
        use_filename: false,
        unique_filename: false,
      });
    ...
    // ── Mode: galeri video broadcast (Admin Abuse) — video WebM transparan ──
    if (uploadType === 'broadcast-video') {
