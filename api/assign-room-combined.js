...
/* ==========================
   ...
   CATATAN PENTING: handleUpload dari @vercel/blob mengharapkan object
   `request` bertipe Web API Request standar (yang punya method .json(), dst),
   BUKAN object `req` gaya lama (Express/Pages API). Karena project ini pakai
   gaya lama (req, res), kita bikin ulang Request standar secara manual di sini.
   ========================== */
async function handleEventUploadToken(req, res) {
  try {
    const parsedBody = typeof req.body === 'string' ? JSON.parse(req.body) : req.body;

    const fakeRequest = new Request(`https://${req.headers.host}${req.url}`, {
      method: 'POST',
      headers: req.headers,
    ...
    console.error("event upload token error:", err);
...
export default async function handler(req, res) {
  try {
    ...
    const bodyForCheck = typeof req.body === 'string' ? (() => { try { return JSON.parse(req.body); } catch { return null; } })() : req.body;
    if (req.method === "POST" && bodyForCheck && bodyForCheck.type === "blob.generate-client-token") {
