...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!saSession || !saSession.active) {
      return res.status(403).json({ error: 'Akses admin tidak valid', debug: { adminToken: adminToken?.substring(0, 20), saSession } });
    }
  } catch (e) {
    return res.status(500).json({ error: 'Gagal verifikasi sesi admin: ' + e.message, stack: e.stack });
  }
