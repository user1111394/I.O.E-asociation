...
async function saLoadAllMembers() {
  ...
  if (!token) {
    document.getElementById('sa-member-list').innerHTML =
      '<p style="color:#ff6b6b; font-size:12px; text-align:center; padding:20px;">DEBUG: Token tidak ditemukan di localStorage</p>';
    return;
  }
  ...
  try {
    ...
    if (data.success) {
      ...
      const debugInfo = data.debug ? `<br><small style="color:#718096;">DEBUG: ${saEscHtml(JSON.stringify(data.debug))}</small>` : '';
      document.getElementById('sa-member-list').innerHTML =
        `<p style="color:#ff6b6b; font-size:12px; text-align:center; padding:20px;">${saEscHtml(data.error || 'Gagal memuat data')}${debugInfo}</p>`;
    }
  } catch (e) {
    document.getElementById('sa-member-list').innerHTML =
      `<p style="color:#ff6b6b; font-size:12px; text-align:center; padding:20px;">Gagal terhubung ke server: ${saEscHtml(e.message)}</p>`;
  }
}
