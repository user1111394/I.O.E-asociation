...
function saRenderQuestList() {
  ...
  listEl.innerHTML = saQuestDraft.map((q, idx) => `
    <div style="background:rgba(255,255,255,0.04); border:1px solid rgba(255,90,160,0.2); border-radius:8px; padding:10px;">
      ...
      <select onchange="saUpdateQuestField('${q.id}','type',this.value)" style="width:100%; padding:8px; margin-bottom:6px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,90,160,0.2); border-radius:6px; color:#fff; font-size:12px; box-sizing:border-box;">
        <option value="visit" ${q.type === 'visit' ? 'selected' : ''}>Kunjungi (tombol saja)</option>
        <option value="submit-link" ${q.type === 'submit-link' ? 'selected' : ''}>Salin Tautan Video</option>
      </select>
      ...
      <input type="number" min="1" placeholder="Reward koin" value="${q.rewardCoin}" oninput="saUpdateQuestField('${q.id}','rewardCoin',parseInt(this.value,10)||0)" style="width:100%; padding:8px; background:rgba(255,255,255,0.05); border:1px solid rgba(255,90,160,0.2); border-radius:6px; color:#fff; font-size:12px; box-sizing:border-box;">
    </div>
  `).join('');
}
...
function saSwitchEventSubtab(subtab) {
  const subs = { acara: 'sa-eventsub-acara', koin: 'sa-eventsub-koin', broadcast: 'sa-eventsub-broadcast', galeri: 'sa-eventsub-galeri' };
  const tabs = { acara: 'sa-subtab-acara', koin: 'sa-subtab-koin', broadcast: 'sa-subtab-broadcast', galeri: 'sa-subtab-galeri' };
