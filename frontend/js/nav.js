...
function openSuperadminPanel() {
  ...
  box.innerHTML = `
    ...
    <div id="sa-panel-event" style="display:none;">
      ...
      <label style="display:block; font-size:11px; color:#718096; margin-bottom:6px;">Upload file (boleh pilih lebih dari satu — gambar, PDF, dll)</label>
      <input id="sa-event-files" type="file" multiple style="width:100%; margin-bottom:10px; font-size:12px; color:#cbd5e0;">
...
function saSwitchTab(tab) {
  const panels = { stats: 'sa-panel-stats', member: 'sa-panel-member', tos: 'sa-panel-tos', event: 'sa-panel-event' };
  const tabs = { stats: 'sa-tab-stats', member: 'sa-tab-member', tos: 'sa-tab-tos', event: 'sa-tab-event' };
  const activeColors = {
...
const EVENT_FIREBASE_CONFIG = {
  apiKey:            "ISI_API_KEY_FIREBASE_BARU",
  authDomain:        "ISI_AUTH_DOMAIN_BARU",
  databaseURL:       "ISI_DATABASE_URL_BARU",
  projectId:         "ISI_PROJECT_ID_BARU",
  messagingSenderId: "ISI_SENDER_ID_BARU",
  appId:             "ISI_APP_ID_BARU"
};
...
async function saCreateEvent() {
  ...
  try {
    // Upload file lewat server kita (api/event-upload.js) yang PUT langsung ke
    // REST API Vercel Blob pakai fetch native — tidak pakai package @vercel/blob
    // sama sekali, karena package itu berkali-kali gagal ke-install di server.
    ...
    for (let i = 0; i < files.length; i++) {
      statusEl.textContent = `Upload file ${i + 1}/${files.length}...`;
      const file = files[i];
      try {
        ...
        fileUrls.push({ name: file.name, url: uploadData.url });
      } catch (uploadErr) {
        throw new Error(`Upload "${file.name}" gagal — ${uploadErr.message || uploadErr}`);
      }
    }
    ...
    statusEl.textContent = '❌ Gagal membuat event. Cek console untuk detail.';
    statusEl.style.color = '#ff6b6b';
  } finally {
    btn.disabled = false;
    ...
    listEl.innerHTML = activeEvents.map(ev => {
      ...
      return `
        <div style="background:rgba(255,255,255,0.04); border:1px solid rgba(255,90,160,0.2); border-radius:10px; padding:12px;">
          ...
          <div style="font-size:10px; color:#718096; margin-top:6px;">📎 ${ev.files ? ev.files.length : 0} file · ⏳ ${daysLeft} hari lagi</div>
        </div>
      `;
    }).join('');
  } catch (err) {
    console.error('Gagal memuat daftar event:', err);
    listEl.innerHTML = '<p style="color:#ff6b6b; font-size:12px; text-align:center; padding:20px;">Gagal memuat event.</p>';
  }
}
