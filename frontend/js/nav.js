...
async function saCreateEvent() {
  ...
  try {
    // Upload file langsung dari browser ke Vercel Blob (tidak lewat server kita,
    // endpoint /api/assign-room cuma kasih token sementara — lihat bagian
    // EVENT UPLOAD TOKEN di assign-room.js)
    ...
    for (let i = 0; i < files.length; i++) {
      ...
      const blob = await upload(`events/${eventId}/${file.name}`, file, {
        access: 'public',
        handleUploadUrl: '/api/assign-room',
      });
      fileUrls.push({ name: file.name, url: blob.url });
    }
