...
// ═══ SUB-TAB SWITCHER: Acara vs Koin (di dalam tab Event) ═══
function saSwitchEventSubtab(subtab) {
  const subs = { acara: 'sa-eventsub-acara', koin: 'sa-eventsub-koin' };
  const tabs = { acara: 'sa-subtab-acara', koin: 'sa-subtab-koin' };
    ...
    listEl.innerHTML = items.map(item => `
      <div style="display:flex; align-items:center; gap:10px; padding:10px; background:rgba(255,255,255,0.03); border:1px solid rgba(255,255,255,0.08); border-radius:8px;">
        ...
        <div style="flex:1; min-width:0;">
          ...
          <div style="font-size:11px; color:#ffd23f;">${item.koinType === 'event' ? '🎉' : '🪙'} ${item.harga}${item.koinType === 'event' ? ' <span style="color:#ff5aa0;">(event)</span>' : ''}</div>
        </div>
