...
async function saCreateEvent() {
  ...
  try {
    ...
    await set(ref(db, `events/${eventId}`), {
      ...
      // Disimpan supaya nanti bisa dipakai untuk menghapus file dari
      // Cloudinary saat event ini dihapus (lihat saDeleteEvent).
      challengePathname: uploadData.pathname,
      createdAt: now,
      expiresAt,
      days
    });
...
async function saUploadGalleryFile(type) {
  ...
  btn.disabled = true;
  btn.textContent = 'Mengupload...';
  statusEl.textContent = 'Mengupload ke Cloudinary...';
  statusEl.style.color = '#718096';
  ...
  try {
    ...
    const uploadType = type === 'music' ? 'broadcast-music' : 'broadcast-video';
    const uploadRes = await fetch('/api/event-upload', {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({
    ...
    // Simpan ke daftar galeri lewat action generik di shop.js
    const adminToken = saGetAdminToken();
