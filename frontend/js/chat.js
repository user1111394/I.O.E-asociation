...
async function init() {
  try {
    ...
    let myKey  = '';
    let myMemberId = '';
    let myFoto = ''; // fotoProfil dari akun.html, dipakai sebagai avatar bulat di chat
    ...
    window.handleJoinAction = async function() {
      ...
      set(presRef, { name: myName, color: myColor, memberId: myMemberId, foto: myFoto, joinedAt: serverTimestamp() });
      onDisconnect(presRef).remove();

      // Update counter Redis: +1 online di room ini
      fetch('/api/assign-room', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ roomId: myRoomId, action: 'join' })
      }).catch(e => console.error('Gagal update counter join:', e));

      // Kurangi counter Redis pas tab ditutup/keluar (best-effort, browser)
      window.addEventListener('beforeunload', () => {
        navigator.sendBeacon('/api/assign-room', new Blob(
          [JSON.stringify({ roomId: myRoomId, action: 'leave' })],
    ...
    window.sendMsg = function() {
      ...
      push(ref(db, `rooms/${myRoomId}/messages`), {
        type: 'msg', name: myName, color: myColor, memberId: myMemberId, foto: myFoto,
        text: txt, ts: serverTimestamp()
      });
      fetch('/api/assign-room', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ roomId: myRoomId, action: 'message' })
      }).catch(e => console.error('Gagal update counter message:', e));
    };

    window.sendReaction = function(emoji) {
      hideReactBar();
      push(ref(db, `rooms/${myRoomId}/messages`), {
        type: 'reaction', name: myName, color: myColor, memberId: myMemberId, foto: myFoto,
      ...
      onValue(msgsRef, snap => {
        ...
        toShow.forEach(m => {
          if (m.type === 'system' || m.type === 'reaction') {
            ...
            const aviInner = m.foto
              ? `<img src="${m.foto}" alt="" style="width:100%;height:100%;object-fit:cover;border-radius:50%;">`
              : initials;
            wrap.innerHTML = `
              <div class="chat-avi" style="background:${m.color}20;border:1px solid ${m.color}40;color:${m.color}">${aviInner}</div>
