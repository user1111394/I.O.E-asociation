    ...
    window.pcOpenChat = function(chatId, otherUserId, otherUsername, otherFoto) {
      ...
      msgListenerUnsub = onValue(msgsRef, (snap) => {
        ...
        container.innerHTML = msgs.map(([id, m]) => {
          ...
          // DEBUG SEMENTARA — tampil kecil di bawah tiap bubble, HAPUS sebelum rilis.
          const debugLine = `<div style="font-size:8px;opacity:0.5;color:#ff5aa0;">senderId:${escHtml(String(m.senderId||'∅'))} vs me:${escHtml(String(currentMemberId||'∅'))} → ${mine}</div>`;
          return `<div class="pc-msg ${mine ? 'pc-msg-mine' : 'pc-msg-theirs'}">
            <div class="pc-msg-avatar">${aviInner}</div>
            <div class="pc-msg-bubble">${escHtml(m.text || '')}${debugLine}</div>
          </div>`;
        }).join('');
