  ...
  try {
    ...
    window.pcSearchUsername = async function() {
      ...
      try {
        ...
        const searchChatId = makeChatId(myMemberId, data.memberId);
        const safeSearchChatId = searchChatId.replace(/'/g, "\\'");
        const safeSearchName = escHtml(data.cleanUsername).replace(/'/g, "\\'");

        resultEl.innerHTML = `
          <div class="pc-chat-item-wrap">
            <button class="pc-chat-item" onclick="window.pcOpenChat('${searchChatId}','${data.memberId}','${escHtml(data.cleanUsername)}','${searchFotoAttr}')">
            ...
            <button class="pc-chat-delete-btn" onclick="event.stopPropagation(); window.pcDeleteChat('${safeSearchChatId}','${safeSearchName}')" aria-label="Hapus percakapan">🗑️</button>
          </div>
        `;
      } catch (e) {
    ...
    window.pcOpenChat = function(chatId, otherUserId, otherUsername, otherFoto) {
      ...
      msgListenerUnsub = onValue(msgsRef, (snap) => {
        ...
        container.innerHTML = msgs.map(([id, m]) => {
          const mine = String(m.senderId || '').trim() === String(myMemberId || '').trim();
