    ...
    window.pcOpenChat = function(chatId, otherUserId, otherUsername, otherFoto) {
      ...
      msgListenerUnsub = onValue(msgsRef, (snap) => {
        ...
        container.innerHTML = msgs.map(([id, m]) => {
          ...
          // Inline style dipakai langsung (bukan cuma andalkan class CSS) supaya
          // posisi kiri/kanan dijamin selalu benar, tidak tergantung cascading.
          const rowStyle = mine
            ? 'display:flex;align-items:flex-end;gap:6px;justify-content:flex-end;flex-direction:row-reverse;'
            : 'display:flex;align-items:flex-end;gap:6px;justify-content:flex-start;';
          return `<div class="pc-msg ${mine ? 'pc-msg-mine' : 'pc-msg-theirs'}" style="${rowStyle}">
